...
class Utils:
    ...
    def give_contacts(self, login):
        return {
            "response": 202,
            "alert": [get_user_login(contact.client_id) for contact in get_contacts(get_user_id(login))]
        }
    ...
    def get_history(self, login, contact_login):
        ...
        try:
            return {
                "response": 203,
                "messages": [[message.id, get_user_login(message.from_user_id), message.message_text] for message in
                             get_users_messages(user_id, contact_id)],
            }
        except:
            return []
    ...
    def handle_message(self, message):
        if "response" in message:
            ...
            if message["action"] == "presence":
                ...
                try:
                    return self.get_history(message['login'], message['contact_login'])
                except:
