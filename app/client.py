...
from db.commands import get_all_users, add_user, get_chat_id, get_user_id, get_contacts, get_user_login, \
    get_users_messages
...
class Client(Utils):
    ...
    @logs
    def client_registration(self, login, info):
        try:
            add_user(login, info)
            return True
        except:
    ...
    def get_history(self, login, contact_login):
        try:
            return [[get_user_login(message.from_user_id), message.message_text] for message in get_users_messages(get_user_id(login), get_user_id(contact_login))]
            # message = {
            #     "action": "get_history",
            #     'login': login,
            #     'contact_login': contact_login
