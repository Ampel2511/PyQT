...
from db.db_server import session, Contacts, History, Chats, Users
...
def add_user(login, info, pswd):
    try:
        old_user = get_user_id(login)
        if old_user:
            new_login = input(f'Пользователь с логином {login} уже существует, введите другой логин: ')
            add_user(new_login, info, pswd)
        print(f'добавление пользователя {login, info, pswd}')
        session.add(Users(login, info, pswd))
        session.commit()
        return True
    except:
...
def get_user_info(ip):
    try:
        return session.query(Users).filter_by(ip=ip).first()
    except:
        return False
...
def create_chat(user_id, contact_id):
    try:
        if get_chat_id(user_id, contact_id):
            return True
...
def get_chat_id(user_id, contact_id):
    try:
        ...
        print('Возникла ошибка при получении чата')
        return False
...
def get_users_messages(user_id, contact_id):
    try:
        ...
        print('Возникла ошибка при получении сообщений')
        return []
